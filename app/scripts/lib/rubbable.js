...
var RubbableGif = module.exports = function( options ) {
  ...
  var registerCanvasHandlers = function () {
    ...
    var w = ( options.vp_w ? options.vp_w : canvas.width ),
    ...
    $(document).on((cantouch) ? 'touchend' : 'mouseup', function (e) {
      if (startTime) {
        startTime = 0;
        startX = 0;
        startY = 0;
    ...
    canvas.addEventListener((cantouch) ? 'touchmove' : 'mousemove', function (e) {
      ...
      if (startTime !== 0 ) {
        ...
        currentVDistance = (startY === 0) ? 0 : Math.abs(currentY - startY);
        currentTime = e.timeStamp;

        if (cantouch) {
          if (currentVDistance < maxDistance || currentDistance > maxDistance) {
            e.preventDefault();
          }
        }
